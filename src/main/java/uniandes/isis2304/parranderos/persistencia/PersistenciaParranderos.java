...
//        	e.printStackTrace();
	...
	public Cliente adicionarCliente(long numDoc, String tipoDocumento, String nombre, String correo) 
	{
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
	...
	public Habitacion adicionarHabitacion(long id, long idHotel, int tipo, String reservada, String ocupada, int capacidad, double costo) {
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
		{
			tx.begin();
			long tuplasInsertadas = sqlHabitacion.adicionarHabitacion(pm, id, idHotel, tipo, reservada, ocupada, costo, capacidad);
			tx.commit();

			log.trace ("Inserción de bebedor: " + tipo + ": " + tuplasInsertadas + " tuplas insertadas");
			System.out.println("wtf");
			return new Habitacion(id, idHotel, tipo, reservada, ocupada, capacidad, costo);
		}
		catch (Exception e)
		{
			//        	e.printStackTrace();
...
//        	e.printStackTrace();
	...
	public TipoUsuario adicionarTipoUsuario(int pId, String pDesc)
	{
		PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx=pm.currentTransaction();
        try
        {
            tx.begin();
            long tuplasInsertadas = sqlTipoUsuario.adicionarTipoUsuario(pm, pId, pDesc);
            tx.commit();
            
            log.trace ("InserciÃ³n de tipo de usuario: " + pDesc + ": " + tuplasInsertadas + " tuplas insertadas");
            
            return new TipoUsuario (pId, pDesc);
        }
        catch (Exception e)
        {
//        	e.printStackTrace();
	...
	public Usuario adicionarUsuario(long pId, String tipoDocumento, String nombre, String correo, int pTipo, int idHotel){
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
		{
			tx.begin();
			long tuplasInsertadas = sqlUsuario.adicionarUsuario(pm, pId, tipoDocumento, nombre, correo, pTipo, idHotel);
			System.out.println("papi");
			tx.commit();

			log.trace ("Inserciónn de usuario: " + nombre + ": " + tuplasInsertadas + " tuplas insertadas");

			return new Usuario(pId, tipoDocumento, correo, nombre, pTipo, idHotel);
		}
		catch (Exception e)
		{
			//        	e.printStackTrace();
